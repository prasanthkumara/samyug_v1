yapp.controller("UserController", ["$scope", "$location", "$http", function($scope, $location, $http) {
	$http({
	    method: "POST",
	    url: HOST + API_PATH + "/users/list.php",
	    headers: {
...
yapp.controller("UserSignupController", ["$scope", "$location", "$http", function($scope, $location, $http) {

	//signup initialize
	$scope.credential={"email":"","password":"","name":""};
	$scope.error={"email":false,"password":false,"name":false};
	...
	$scope.signUp=function(){
		...
		$http({
	        method: "POST",
	        url: HOST + API_PATH + "/users/signup.php",
